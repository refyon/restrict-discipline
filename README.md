...
| 4 | 操作留痕与复用 | 执行有实际效果的操作前先用 `find_operation` 按关键词检索 `scripts/` 是否已有相同脚本，命中则直接运行复用；未命中再执行，并用 `record_operation` 保存为 `scripts/<会话名>/` 下可双击运行的脚本（`.cmd` + `.ps1`），维护 `index.md` 索引；通用流程（发布、构建、CI 触发等）首次执行必记录，后续会话优先复用 |
| 5 | 目录外确认 | 修改/删除项目目录以外的文件时请求用户确认；只读访问默认放行 |
| 6 | 会话摘要与跨会话复用 | 每轮对话结束自动将会话总结（含本会话已记录脚本清单，自动过滤运行时噪音）写入 `memory/<会话名>.txt`（覆盖旧摘要）；新会话开始时先读取历史摘要，复用之前会话执行过的脚本（配合规则 4 的 `find_operation`） |
...
| 12 | 避免重复验证 | 状态未变化时不重复执行/读取；确需复验时合并为一条命令完成 |

### 编码纪律（软约束，不拦截）

| # | 规则 | 说明 |
|---|------|------|
| 13 | Karpathy 编码准则 | 编码/改动前遵循 [andrej-karpathy-skills](https://github.com/multica-ai/andrej-karpathy-skills)：(1) 先想后写——明确假设、有歧义先问、说明权衡；(2) 简单优先——只写最少必要代码；(3) 外科手术式修改——只改必须改的、保持既有风格；(4) 目标驱动——转成可验证标准并循环验证 |

### Token 节约补充（软约束，不拦截）

| # | 规则 | 说明 |
|---|------|------|
| 14 | 批量编辑 | 同一文件的多次小修改合并为一次 read + 多次 edit；相关改动合并为一次 git 提交 |
| 15 | 失败收敛 | 同一目标操作因沙箱/环境失败时最多尝试 2 种替代方式，之后停止并询问用户或改策略 |
