...
const ENV_TOKEN_RE = /\.env(?![a-zA-Z0-9_.-])/i
...
export function buildEnforcer(deps) {
  ...
  return async function enforce(exec) {
    ...
    if (await isEnvAttempt(name, args, root)) {
      ...
      return { kind: 'deny', reason: '【restrict-discipline】检测到对项目根目录 .env 文件的访问/修改/删除尝试（工具：' + name + '）。已阻止，并已记录到 log/ 目录。请立即在回复中告知用户发生了此事件。' }
    }
    ...
    if (name === 'pwsh') {
      ...
      if (PROXY_PATTERNS.some(re => re.test(cmd))) {
        ...
        return { kind: 'deny', reason: '【restrict-discipline】禁止修改系统代理设置或 git/npm 等工具的 proxy 配置。已阻止该命令。' }
      }
      // Rule 1（pwsh 途径）：根目录直接建文件
      if (rootWriteCheck(root)(cmd)) {
        return { kind: 'deny', reason: '【restrict-discipline】禁止在项目根目录创建文件。请把文件放入对应的分类子目录（如 scripts/、log/）。' }
      }
    }
    ...
    if (name === 'write' && typeof args.file_path === 'string') {
      ...
      if (abs && lower(dirname(abs)) === nRoot) {
        return { kind: 'deny', reason: '【restrict-discipline】禁止在项目根目录创建文件：' + args.file_path + '。请放入对应的分类子目录（如 scripts/、log/）。' }
      }
    }
    ...
    if (outside) {
      return { kind: 'ask', reason: '【restrict-discipline】检测到对项目目录以外文件的修改/删除：' + outside + '（工具：' + name + '）。是否允许执行？' }
    }
