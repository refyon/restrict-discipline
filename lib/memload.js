// lib/memload.js — memory/MEMORY.md 项目记忆的纯逻辑（无依赖、可独立测试）。
//
// 设计要点：
//  1. 文件即记忆：会话启动时全量加载 memory/MEMORY.md（确定性加载，无检索），
//     超 maxLoadChars（默认 25KB）截断并注记。
//  2. # 快捷方式：文件中一级标题是记忆条目；用户消息里的「#条目名」触发定向召回，
//     把对应条目区块注入上下文。
//  3. 路径固定：本模块只认 memory/MEMORY.md 这一个路径，杜绝模型猜路径。

/** 默认注入上限（字符数）≈ 25KB。 */
export const DEFAULT_MAX_CHARS = 25600

/** 切分项目记忆文本为条目：一级标题 = 条目 token；首个一级标题之前的内容为前导头。 */
...
/** 渲染项目记忆注入块（会话启动用）。text 为空时返回 ''。legacy=true 时附加旧文件名重命名提示。 */
export function renderLoadBlock(text, maxChars, legacy = false) {
  const { text: body, truncated, total } = capLoad(text, maxChars)
  if (!body.trim()) return ''
  const header = '【项目记忆 memory/MEMORY.md（restrict-discipline 自动注入，需要细节时直接 read 该文件；用户消息中含「#条目名」可定向召回对应条目）】'
  const legacyNote = legacy ? '\n\n（提示：项目记忆当前仍以旧文件名 memory/CLAUDE.md 存放，建议重命名为 memory/MEMORY.md）' : ''
  const note = truncated ? '\n\n（文件共 ' + total + ' 字符，超出注入上限已截断到 ' + body.length + ' 字符；剩余部分请按需 read）' : ''
  return header + legacyNote + '\n' + body + note
}

/** 渲染 # 快捷召回块。hits 为 recallSections 结果；无命中返回 ''。 */
export function renderRecallBlock(hits, token) {
  if (!Array.isArray(hits) || hits.length === 0) return ''
  const header = '【# 快捷召回：' + token + '（memory/MEMORY.md 条目，restrict-discipline 注入）】'
