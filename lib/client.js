    ...
    function Card(props) {
      ...
      return createElement(
        ...
        createElement('div', { style: DESC_STYLE },
          '四元行为规范（强制约束/Token 节约/会话记忆/编码纪律）：禁止项目根目录建文件、保护根目录 .env、禁止修改代理设置、目录外修改需确认、操作留痕，以及 Claude Code 式项目记忆 memory/CLAUDE.md（启动自动加载、# 快捷召回、remember 追加）。',
        ),
      ...
      ctx.effect(function () {
        return slots.inject('settings.plugin.item', function () {
          return slots.register(
            { name: 'settings.plugin.item', key: 'restrict-discipline', order: 30 },
