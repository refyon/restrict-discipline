...
//
// 规则（4 条主规则，细规则收编为子项；文本见 lib/rules.js）：
...
//   3. 会话记忆（3.1 项目记忆文件自动加载+remember 追加 3.2 # 快捷召回 3.3 会话摘要 3.4 无检索工具）
//   4. 编码纪律（Karpathy 准则）
...
const Schema = z.object({
  ...
  memoryTools: z.boolean().default(true), // remember 工具开关
  autoLoad: z.boolean().default(true), // 会话启动自动加载 memory/MEMORY.md（3.1）
...
export function apply(ctx) {
  ...
  // ---- 项目记忆文件：新名 MEMORY.md；旧名 CLAUDE.md 在新名不存在时回退读取（兼容过渡）----
  const MEM_FILE = 'MEMORY.md'
  const MEM_LEGACY = 'CLAUDE.md'
  const readProjectMem = async (agent) => {
    const cur = await readMemFile(agent, MEM_FILE)
    if (String(cur).trim()) return { text: cur, legacy: false }
    const old = await readMemFile(agent, MEM_LEGACY)
    if (String(old).trim()) return { text: old, legacy: true }
  ...
  // 读 memory/MEMORY.md（不存在时回退读旧名 CLAUDE.md）→ agent.inject() 以 user 消息注入
  // （logged channel，随历史保留；框架同类先例：dsh-session 文档明示 agent.inject() 用于
  // subdir AGENTS.md 等合成上下文）。路径固定：记忆路径由系统注入，不让模型猜。
  // global:true：host 层监听 agent-scoped 事件必须 global。
    ...
    try {
      ...
      void (async () => {
        try {
          ...
          const mem = await readProjectMem(agent)
          if (!String(mem.text || '').trim()) return
          const block = renderLoadBlock(mem.text, cfg('maxLoadChars', DEFAULT_MAX_CHARS), mem.legacy)
          if (!block) return
          agent.inject({
  ...
  // ---- 规则 3.2：# 快捷召回 ----
  // 用户消息含「#条目名」时，把项目记忆文件中对应一级标题区块注入会话表面。
    ...
    try {
      ...
      void (async () => {
        try {
          ...
          const target = await fsSvc.resolve(join(root, 'memory\\' + MEM_FILE), { cwd: root })
          let content = ''
          try { content = await fsSvc.readText(target) } catch (e) { /* 无新名文件 */ }
          if (!String(content).trim()) {
            try {
              const legacyTarget = await fsSvc.resolve(join(root, 'memory\\' + MEM_LEGACY), { cwd: root })
              content = await fsSvc.readText(legacyTarget)
            } catch (e) { return }
          }
          const hits = []
  ...
  if (tools !== undefined) {
    ...
    // ---- remember —— 追加到项目记忆文件「## 记忆条目」分节 ----
    // 项目记忆为单文件：每次会话启动自动注入（规则 3.1），agent 可直接 read/edit 维护；
    // remember 是唯一记忆工具，写入路径固定（杜绝模型猜路径）。
    ...
    reg({
      name: 'remember',
      description: '把需要长期记住的决策/偏好/结论追加到 memory/MEMORY.md 的「## 记忆条目」分节（Markdown 项目记忆；该文件在每次新会话启动时自动注入上下文）。写入自动带时间戳与来源审计（会话 id · 轮次），并脱敏密钥/令牌。只记需要跨会话复用的信息；文件其余内容可用 read/edit 直接维护。',
      parameters: {
        type: 'object',
        properties: {
      ...
      async execute(args, exec) {
        ...
        return memLock(async () => {
          const mem = await readProjectMem(agent)
          const text = appendMemoryEntry(mem.text, line)
          await writeFile(join(root, 'memory\\' + MEM_FILE), text, root, agent)
          return '已记住 ' + id + ' → memory\\' + MEM_FILE
