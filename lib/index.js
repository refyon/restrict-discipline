...
//
// 规则（1–7 强制；8–12 Token 节约软约束；13–14 Token 节约补充软约束；15 Karpathy 编码准则软约束，均不拦截）：
//   1. 禁止在项目根目录创建文件
//   2. 禁止访问/修改/删除项目根目录下的 .env
//   3. 禁止修改系统代理设置或 git/npm 等工具的 proxy 配置
//   4. 操作留痕与复用：find_operation 先查复用，record_operation 保存为 scripts/<会话名>/ 下可双击脚本
//   5. 修改/删除项目目录外的文件需用户确认（只读默认放行）
//   6. 会话摘要与跨会话复用：摘要写入 memory/<会话名>.txt（含脚本清单、过滤噪音）
...
//  12. 避免重复验证：无变化状态不重复执行/读取
//  13. 批量编辑：一次 read + 多次 edit；相关改动合并一次 git 提交
//  14. 失败收敛：失败最多试 2 种替代方式，之后询问用户或改策略
//  15. Karpathy 编码准则：先想后写 / 简单优先 / 外科手术式修改 / 目标驱动
...
import { rankMemoryFiles, renderMemoryBlock, buildRecallNotice } from './memory.js'
...
const RULES_TEXT = '【行为规范 - 由 restrict-discipline 插件强制，可在设置→插件中关闭】\n'
  ...
  + '6. 会话摘要与跨会话复用：每轮对话结束时，插件自动把当前会话总结写入 memory/<会话名>.txt（覆盖旧摘要），其中包含本会话已记录脚本清单，并自动过滤运行时噪音（runtime context 快照、system-reminder、checkpoint 等）。新会话首次内容解析时插件已自动注入相关历史记忆（见规则 16），无需手动读取 memory/ 目录；执行相同操作前先调用 find_operation 检索 scripts/ 下是否已有脚本，命中则直接复用。\n'
  ...
  + '【编码纪律 - Andrej Karpathy 准则（软约束，不拦截，来源 https://github.com/multica-ai/andrej-karpathy-skills）】\n'
  + '15. 编码/改动前遵循 Karpathy 准则：(1) 先想后写——不臆测，明确说出假设，有歧义或困惑即先提问，说明权衡，存在更简单做法时主动提出；(2) 简单优先——只写解决问题所需的最少代码，不写未被要求的特性、抽象或配置，200 行能压成 50 行就重写；(3) 外科手术式修改——只改必须改的行，保持既有风格，不顺手重构或"改进"无关代码，只清理自己造成的死代码；(4) 目标驱动——把任务转成可验证的成功标准并循环验证，多步任务先列分步计划与每步验证方式。\n'
  + '【会话记忆（软约束，不拦截，由插件自动执行）】\n'
  + '16. 会话记忆自动注入：新会话的首次内容解析时，插件已自动检索 memory/ 中与当前任务最相关的历史会话摘要并注入上下文（见【历史记忆】块，每会话仅注入一次）。直接复用注入内容即可，后续对话不需要再次回忆，也不要重复读取 memory/ 目录下的历史摘要文件；确需完整细节时再 read 对应文件。'
...
export function apply(ctx) {
  ...
  // ---- 规则 16：新会话首次内容解析自动注入相关历史记忆（每会话仅一次） ----
  ...
  ctx.on('system-prompt/assemble', async (assembly, context, next) => {
    ...
    try {
      ...
      let entries
      try {
        const memDir = await fsSvc.resolve(join(root, 'memory'), { cwd: root })
        entries = await fsSvc.listDir(memDir)
      } catch (e) {
        return assembled
      }
      const files = []
      for (const e of entries || []) {
        if (!e || e.type !== 'file' || !/\.txt$/i.test(e.name)) continue
        try {
          files.push({ name: e.name, text: await fsSvc.readText(e.target) })
        } catch (err) { /* 跳过不可读文件 */ }
      }
      const selected = rankMemoryFiles(files, query, { limit: 3, excludeName: sessionNameOf(agent) + '.txt' })
