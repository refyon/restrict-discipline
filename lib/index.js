...
//   6. 会话摘要：record_summary 写入 memory/<会话名>.txt（覆盖），轮次结束自动兜底
...
const RULES_TEXT = '【行为规范 - 由 restrict-discipline 插件强制，可在设置→插件中关闭】\n'
  ...
  + '4. 操作留痕：每完成一个有实际效果的操作（安装依赖、构建、写文件、git 提交、网络请求、批处理等），必须调用 record_operation 工具，把实际执行的命令保存为 scripts/<会话名>/ 下可双击运行的脚本（提供 command、description，必要时 workdir）。纯查询命令（dir、git status、Get-Process 等）不需要记录。\n'
  + '5. 目录外确认：当需要修改或删除项目目录以外的文件（write/edit 工具，或 pwsh 的写入/删除命令）时，插件会弹出确认请求，你应等待用户确认后再继续；只读访问（read/grep/glob/读取命令）默认放行，无需确认。\n'
  + '6. 会话摘要：每轮对话结束时，调用 record_summary 工具，把当前会话（含本轮新产生的对话内容）总结后写入 memory/<会话名>.txt（覆盖旧摘要）。若你未调用，插件会在轮次结束时自动生成结构化摘要兜底。\n'
...
export function apply(ctx) {
  ...
  let seq = 0
  const summarized = new Map() // agentId -> 本轮已总结
  let settingsScope
  ...
  // ---- 规则 6：轮次结束自动摘要兜底（status -> idle 时，若助手未手动总结） ----
  ctx.on('agent/status', (payload) => {
    try {
      ...
      const already = summarized.get(id) === true
      summarized.set(id, false)
      if (already) return
      void (async () => {
        try {
          await writeMemory(agent, buildDigest(agent))
        } catch (err) {
    ...
    try {
      ...
      for (const ev of events) {
        ...
        if (ev.type === 'user/message') {
          ...
          if (t) recent.push('[用户] ' + t)
        } else if (ev.type === 'assistant/message') {
          assistantCount++
          const t = textOf(ev.data && ev.data.message && ev.data.message.content)
          if (t) recent.push('[助手] ' + t)
        } else if (ev.type === 'tool/result') {
          toolCount++
        }
      }
    } catch (e) { /* ignore */ }
    const lines = recent.slice(-10)
    return '# 会话摘要 — ' + title + '\n'
      + '会话 ID: ' + id + '\n'
      + '更新时间: ' + new Date().toISOString() + '\n'
      + '摘要来源: restrict-discipline 自动生成（助手未在当轮调用 record_summary）\n'
  ...
  if (tools !== undefined) {
    ...
    ctx.effect(() => tools.register({
      name: 'record_summary',
      description: '把当前会话（含本轮新产生的对话内容）总结为文本，覆盖写入 memory/<会话名>.txt。每轮对话结束前都应调用本工具一次，content 传完整的会话总结。',
      parameters: {
        type: 'object',
        properties: {
          content: { type: 'string' },
      ...
      async execute(args, exec) {
        requireEnabled('record_summary')
        const agent = exec && exec.agent
        const root = rootOf(agent)
        if (!fsSvc || !policySvc || !root) {
          throw new Error('record_summary 不可用：缺少 fs/sandboxPolicy 服务或无法确定工作目录')
        }
        const content = String(args.content || '').trim()
        if (!content) throw new Error('content 不能为空')
        const sName = sessionNameOf(agent)
        let id = 'unknown'
        try { if (agent && agent.session && agent.session.id) id = String(agent.session.id) } catch (e) { /* ignore */ }
        const body = '# 会话摘要 — ' + sName + '\n'
          + '会话 ID: ' + id + '\n'
          + '更新时间: ' + new Date().toISOString() + '\n'
          + '摘要来源: 助手手动记录（record_summary / restrict-discipline）\n'
          + '\n' + content + '\n'
        await writeMemory(agent, body)
        if (agent && agent.id) summarized.set(String(agent.id), true)
        return '已更新会话摘要：memory\\' + sName + '.txt'
      },
    }))
  }
