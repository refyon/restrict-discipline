...
import { splitFile, rebuildFile, appendEntry, forgetEntry, purgeArchived, countEntries, entryLine, firstTitle } from './memfile.js'
...
export function apply(ctx) {
  ...
  /** 列表 memory/ 下全部记忆文档 → [{ name, text, ts }]（Markdown 文档）。 */
  const listMemFiles = async (agent) => {
    ...
    try {
      ...
      for (const e of entries || []) {
        if (!e || e.type !== 'file' || !/\.md$/i.test(e.name)) continue
        try {
  ...
  /** 判断文件名是否属于当前会话自己的记忆文档（防自我回放）：按稳定会话 id 前缀匹配。 */
  const isSelfMem = (agent) => (nm) => {
    ...
    return !!(idPrefix && n.startsWith(idPrefix + '_'))
  }
  ...
  if (tools !== undefined) {
    ...
    reg({
      name: 'recall_memory',
      description: '按关键词在 memory/*.md 记忆库检索历史记忆（BM25 + 时间衰减；含本会话条目与其他会话摘要），返回命中文件 + 证据行。与规则 3.1 注入的【历史记忆】索引配合做下钻：索引里只有一行摘要，细节用本工具检索。',
      parameters: {
        type: 'object',
        properties: {
