...
//
// 规则（1–7 强制，8–12 Token 节约软约束，13 Karpathy 编码准则软约束，均不拦截）：
//   1. 禁止在项目根目录创建文件
...
//  13. Karpathy 编码准则：先想后写 / 简单优先 / 外科手术式修改 / 目标驱动
...
const RULES_TEXT = '【行为规范 - 由 restrict-discipline 插件强制，可在设置→插件中关闭】\n'
  ...
  + '4. 操作留痕与复用：执行有实际效果的操作（安装依赖、构建、写文件、git 提交、网络请求、批处理等）前，先调用 find_operation 按关键词检索 scripts/ 下是否已有相同脚本；命中则直接运行已有脚本复用（省去重复执行与记录），未命中再执行，完成后调用 record_operation 把实际命令保存为 scripts/<会话名>/ 下可双击运行的脚本（提供 command、description，必要时 workdir）。纯查询命令（dir、git status、Get-Process 等）不需要记录。\n'
  + '5. 目录外确认：当需要修改或删除项目目录以外的文件（write/edit 工具，或 pwsh 的写入/删除命令）时，插件会弹出确认请求，你应等待用户确认后再继续；只读访问（read/grep/glob/读取命令）默认放行，无需确认。\n'
  + '6. 会话摘要与跨会话复用：每轮对话结束时，插件自动把当前会话总结写入 memory/<会话名>.txt（覆盖旧摘要），其中包含本会话已记录脚本清单。新开启的会话开始时，应先读取 memory/ 目录下相关历史摘要（可用 glob memory/*.txt 或 read 打开），了解之前会话的执行经验与可复用脚本；执行相同操作前先调用 find_operation 检索 scripts/ 下是否已有脚本，命中则直接复用。\n'
  ...
  + '13. 编码/改动前遵循 Karpathy 准则：(1) 先想后写——不臆测，明确说出假设，有歧义或困惑即先提问，说明权衡，存在更简单做法时主动提出；(2) 简单优先——只写解决问题所需的最少代码，不写未被要求的特性、抽象或配置，200 行能压成 50 行就重写；(3) 外科手术式修改——只改必须改的行，保持既有风格，不顺手重构或"改进"无关代码，只清理自己造成的死代码；(4) 目标驱动——把任务转成可验证的成功标准并循环验证，多步任务先列分步计划与每步验证方式。'
...
export function apply(ctx) {
  ...
  const buildDigest = async (agent) => {
    ...
    // 读取本会话 scripts/<会话名>/index.md 的脚本清单，方便新会话复用
    let scriptLines = []
    try {
      const root = rootOf(agent)
      if (fsSvc && root) {
        ...
        scriptLines = String(idx).split(/\r?\n/)
          .map(l => l.trim())
          .filter(l => /^- \[.+\.cmd\]/.test(l))
          .slice(0, 20)
      }
