...
//
// 规则（1–7 强制；8–12 Token 节约软约束；13 Karpathy 编码准则软约束；14–15 Token 节约补充软约束，均不拦截）：
//   1. 禁止在项目根目录创建文件
...
//  14. 批量编辑：一次 read + 多次 edit；相关改动合并一次 git 提交
//  15. 失败收敛：失败最多试 2 种替代方式，之后询问用户或改策略
...
const RULES_TEXT = '【行为规范 - 由 restrict-discipline 插件强制，可在设置→插件中关闭】\n'
  ...
  + '14. 批量编辑：同一文件的多次小修改合并为一次 read + 多次 edit，不反复整读；相关改动合并为一次 git 提交，不为每个微调单独提交推送。\n'
  + '15. 失败收敛：同一目标操作因沙箱/环境失败时，最多尝试 2 种替代方式，之后停止并询问用户或改策略，不无限换命令重试。'
